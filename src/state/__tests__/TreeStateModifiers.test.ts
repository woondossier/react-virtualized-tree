...
describe('TreeStateModifiers', () => {
  const noop = (node: Node): Node => node;

  describe('editNodeAt', () => {
    test('should fail when invalid state is supplied', () => {
      expect(() =>
        TreeStateModifiers.editNodeAt('state' as unknown as State, 0, noop)
      ).toThrowError('Expected a State instance but got string');
      expect(() => TreeStateModifiers.editNodeAt(1225 as unknown as State, 0, noop)).toThrowError(
        'Expected a State instance but got number'
      );
      expect(() => TreeStateModifiers.editNodeAt([] as unknown as State, 0, noop)).toThrowError(
        'Expected a State instance but got object'
      );
      expect(() => TreeStateModifiers.editNodeAt({} as unknown as State, 0, noop)).toThrowError(
        'Expected a State instance but got object'
      );
      expect(() => TreeStateModifiers.editNodeAt(true as unknown as State, 0, noop)).toThrowError(
        'Expected a State instance but got boolean'
      );
      expect(() =>
        TreeStateModifiers.editNodeAt((() => {}) as unknown as State, 0, noop)
    ...
    describe('tree', () => {
      ...
      test('should delete state for a root node and keep the rest intact', () => {
        ...
        const { tree } = TreeStateModifiers.editNodeAt(state, 5, (n) => {
          return (Object.keys(n) as (keyof Node)[])
            .filter((k) => k !== 'state')
            .reduce((node, k) => ({ ...node, [k]: n[k] }), {} as Node);
        });
  ...
  describe('deleteNodeAt', () => {
    test('should fail when invalid state is supplied', () => {
      expect(() => TreeStateModifiers.deleteNodeAt('state' as unknown as State, 0)).toThrowError(
        'Expected a State instance but got string'
      );
      expect(() => TreeStateModifiers.deleteNodeAt(1225 as unknown as State, 0)).toThrowError(
        'Expected a State instance but got number'
      );
      expect(() => TreeStateModifiers.deleteNodeAt([] as unknown as State, 0)).toThrowError(
        'Expected a State instance but got object'
      );
      expect(() => TreeStateModifiers.deleteNodeAt({} as unknown as State, 0)).toThrowError(
        'Expected a State instance but got object'
      );
      expect(() => TreeStateModifiers.deleteNodeAt(true as unknown as State, 0)).toThrowError(
        'Expected a State instance but got boolean'
      );
      expect(() => TreeStateModifiers.deleteNodeAt((() => {}) as unknown as State, 0)).toThrowError(
    ...
    test('should fail with descriptive error when node at index does not exist', () => {
      expect(() =>
        TreeStateModifiers.deleteNodeAt(TreeState.createFromTree(Nodes), 20)
      ).toThrowErrorMatchingSnapshot();
    });
