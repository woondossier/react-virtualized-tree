...
import type { Node, FlattenedNode, NodeAction, RendererProps } from './types';
...
interface TestNodeRendererProps extends RendererProps {
  node: FlattenedNode;
...
const TestNodeRenderer = forwardRef<HTMLDivElement, TestNodeRendererProps>(
  ({ node, style, onChange }, ref) => (
...
describe('TreeContainer', () => {
  ...
  describe('memoization', () => {
    ...
    it('should update when nodes change', () => {
      ...
      const newNodes: Node[] = [{ id: 100, name: 'New Node', children: [] }];
      rerender(
  ...
  describe('handleChange callback stability', () => {
    it('should maintain callback stability when dependencies do not change', () => {
      const handleChange = vi.fn();
      const stableNodes = createTestNodes();
      const callbackRefs: ((action: NodeAction) => void)[] = [];

      const CaptureCallback = forwardRef<
        HTMLDivElement,
        { onChange: (action: NodeAction) => void; node: FlattenedNode; style: React.CSSProperties }
      >(({ onChange, node, style }, ref) => {
        useEffect(() => {
      ...
      const { rerender } = render(
        <TreeContainer nodes={stableNodes} onChange={handleChange}>
          {CaptureCallback as React.ComponentType<RendererProps>}
        </TreeContainer>
      );
  ...
  describe('props', () => {
    ...
    it('should work without onChange prop', () => {
      render(<TreeContainer nodes={testNodes}>{TestNodeRenderer}</TreeContainer>);
