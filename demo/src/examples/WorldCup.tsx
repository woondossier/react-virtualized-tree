...
const countries: { [key: number]: CountryInfo } = {
...
const worldCup: Node[] = Object.keys(groups).reduce<Node[]>((wc, g) => {
...
class WorldCupExample extends Component<object, WorldCupState> {
  state: WorldCupState = {
    nodes: worldCup,
  };
    ...
    return (
      <Tree nodes={this.state.nodes} onChange={this.handleChange}>
        {({ style, node, ...rest }: RendererProps) => {
          const countryInfo = countries[node.id as number];
          const country = countryInfo?.flag.toLowerCase();
