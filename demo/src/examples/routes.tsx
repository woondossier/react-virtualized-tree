...
import { totalNumberOfNodes } from './largeCollectionData';
...
const routes: { [key: string]: RouteEntry } = {
  'basic-tree': {
    ...
    description: (
      <div>
        <p>
          A tree that enables favorite toggle, expansion and deletion, this example only makes use
  ...
  'customize-renderers': {
    ...
    description: (
      <div>
        <p>
          A good example of a possible customization of a default renderer is customizing the tree
          to display as a folder structure.
        </p>
        <p>
          By exposing <code>iconsClassNameMap</code> it is possible to pass in the styles applied to
          the Expandable rendererer, the available style options are:
        </p>
        {'{ '}
  ...
  extensions: {
    ...
    description: (
      <div>
        <p>
          A good example of a possible extension is creating a new handler to select nodes that
          automatically selects all the children nodes.
        </p>
        <p>
          By injecting <code>extensions</code> prop with an update type handler for node selection
  ...
  filterable: {
    ...
    description: (
      <div>
        ...
        <p>
          By wrapping the Tree with the <code>FilteringContainer</code> your tree will only recieve
          the nodes it needs to render.
        </p>
      </div>
    ),
  ...
  'node-measure': {
    ...
    description: (
      <div>
        ...
        <p>
          All nodes receive a measure prop that can be used to measure nodes with different heights
          like what happens in this example
        </p>
      </div>
    ),
  ...
  'world-cup': {
    ...
    description: (
      <div>
        <p>
          FIFA world cup is back in 2018, in this special example the tree view is used to display
